...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            sal_finally, rgb_list, dep_list = model(images,depths)
            loss1 = CE(sal_finally, gts)
            ...
            if i % 100 == 0 or i == total_step or i==1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss1.data, loss_cilip.data))
                logging.info('#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f}'.
                    format( epoch, opt.epoch, i, total_step, loss1.data, loss_cilip.data))
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path+'MambaRSOD_epoch_{}.pth'.format(epoch+1))
...
def test(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            image = image.cuda()
            depth = depth.cuda()
            sal_finally, rgb_list, dep_list = model(image,depth)
            res = F.upsample(sal_finally, size=gt.shape, mode='bilinear', align_corners=False)
        ...
        if epoch==1:
            ...
            if mae<best_mae:
                best_mae=mae
                best_epoch=epoch
                torch.save(model.state_dict(), save_path+'MambaRSOD_depth_epoch_best.pth')
                print('best epoch:{}'.format(epoch))
            torch.save(model.state_dict(), save_path + 'MambaRSOD_depth_epoch_last.pth')
